...
- `business_valuation` never averages a meaningless number: a method whose driver (FCF, EBITDA, net income, revenue) is ≤ 0 comes back `null` and is left out of the average (`valuation().usable`, shared with the page), and a call where EBITDA, net income and FCF are all ≤ 0 is rejected with a pointer to `startup_valuation`. The discount rate defaults to the industry's cost of capital; `warnings` flags methods more than 2x apart. Private-company discounts (both valuation tools) come off enterprise value only — never off cash.
- `screen_stocks` may sort by a measure and cap rows (`sort_by` + `order` + `limit`), but only because the user asked: there is no default direction (`order` is required with `sort_by`), and the default stays A–Z. Same rule as the page, where the user can sort by any column.
- `portfolio_analysis` takes an optional `base_currency` (holdings converted at daily Yahoo FX, `GBp` pence handled in `majorCurrency`). VaR and expected shortfall keys carry their period (`var95_daily_pct`, or `_weekly_` on `max`).
- `compound_interest` also returns a PNG chart (`lib/charts.tsx`, drawn with `next/og` — flexbox divs, no chart library). Add charts to other tools there; always return the numbers too.
