...
export const metadata: Metadata = {
  ...
  description: "Plain-English definitions of 32 essential finance terms: WACC, EBITDA, DCF, ROE, RSI, PEG, reverse DCF, alpha, break-even, IRR, NPV and more — including every measure in the FinancePlots stock screener.",
...
interface Term {
  ...
  term: string;
  abbr?: string;
  category: "business" | "personal" | "market" | "stock";
  definition: string;
  example: string;
...
const TERMS: Term[] = [
  ...
  // The stock metrics below used to point at the S&P 500, IBEX 35 and
  // Nasdaq-100 screener pages and quote their filters. Those pages published
  // curated lists and were removed (UK MAR, 2026-09-14), so the examples are
  // now generic and the links go to tools where the user supplies the inputs.
  {
    id: "roe",
    term: "Return on Equity",
    abbr: "ROE",
    category: "stock",
    definition: "Net profit divided by shareholders' equity — how much profit a company generates from the money its owners have put in. A high ROE usually signals a strong competitive position, but it can also be manufactured with debt: borrowing shrinks the equity base and inflates the ratio without the business improving at all. That is why ROE is best read alongside a measure of leverage.",
    ...
    tool: { label: "Stock Screener", href: "/tools/stock-screener" },
  },
  {
    id: "roa",
    ...
    example: "A software company with few physical assets can report an ROA above 15%, while a large bank typically reports around 1% — not because the bank is badly run, but because its balance sheet is almost entirely loans funded by deposits. Comparing the two on ROA says more about their business models than about either company.",
    tool: { label: "Stock Screener", href: "/tools/stock-screener" },
  },
  {
    id: "debt-to-equity",
    ...
    example: "A D/E of 100% means £1 of debt for every £1 of equity. Read next to ROE, it helps separate a high return that comes from the business from one that comes mainly from borrowing.",
    tool: { label: "Stock Screener", href: "/tools/stock-screener" },
  },
  {
    id: "free-cash-flow",
    ...
    example: "A company with £80m of operating cash flow that spends £50m on capital expenditure has free cash flow of £30m. If it reported £60m of profit, only half of that profit turned into cash the business could actually use.",
    tool: { label: "Stock Screener", href: "/tools/stock-screener" },
  },
  {
    id: "peg",
    ...
    example: "A company on a P/E of 12 whose earnings just doubled off a cyclical low shows a PEG of 0.12 — not because it is cheap, but because the growth figure is a one-off rebound. Using multi-year average earnings instead of the last twelve months is the standard fix.",
    tool: { label: "Stock Analysis", href: "/tools/stock-analysis" },
  },
  {
    id: "rsi",
    ...
    example: "A share that has risen on most of the last 14 trading days, with bigger up days than down days, might read 75 — conventionally overbought. Nothing in that reading says whether the company's profits are rising or falling.",
    tool: { label: "Stock Screener", href: "/tools/stock-screener" },
  },
  {
    id: "moving-average",
    ...
    example: "A share trading at £110 with a 200-day average of £100 sits 10% above its long-term trend. A short rally that lifts the price back above its 50-day average, while the 50-day is still below the 200-day, does not change that longer trend.",
    tool: { label: "Stock Screener", href: "/tools/stock-screener" },
  },
  {
    id: "momentum",
    ...
    example: "Six- and twelve-month price returns are the usual way momentum is measured. A value investor would object to leaning on them at all — buying what has already risen is the opposite of buying what is cheap.",
    tool: { label: "Stock Screener", href: "/tools/stock-screener" },
  },
  {
    id: "reverse-dcf",
  ...
  },
  {
    ...
    example: "Cash flows of 100 → 10 → 300 → 700 give an impressive compound growth rate but an R² close to zero: the numbers don't follow a line. Cash flows of 100 → 110 → 121 → 133 grow more slowly with an R² near 1 — a genuine trend.",
    tool: { label: "Business Valuation", href: "/tools/valuation" },
  },
  {
    id: "terminal-value",
    ...
    example: "Applying the same 2.5% terminal growth rate to a miner, a biotech and a software firm assumes they share a long-run growth ceiling, which none of them plausibly does. Checking what share of each valuation the terminal value makes up shows how much that one assumption is carrying.",
    tool: { label: "Business Valuation", href: "/tools/valuation" },
  },
  {
    id: "alpha",
    ...
    example: "A portfolio returning -3.7% while its benchmark returned -0.7% has an alpha of -3.0 percentage points. Judging it on the -3.7% alone tells you about the market; the comparison is what tells you about the portfolio.",
    tool: { label: "Portfolio Analysis", href: "/tools/portfolio-analysis" },
  },
];
...
const CATEGORY_COLORS: Record<Term["category"], string> = {
  ...
  stock: "bg-yellow-500/10 text-yellow-300 border-yellow-500/20",
};
...
export default async function GlossaryPage({ params }: Props) {
  ...
  const subtitle = es
    ...
    : "32 essential finance terms, explained in plain English — including every measure in the stock screener.";
